...
[[ -d $tmpdir ]] || exit 1
cp -ai --parents COPYING src/liblzma $tmpdir
...
find src -type f -name 'Makefile*' -print0 | xargs -0r rm
rm src/liblzma/liblzma* src/liblzma/*.sh
# 5) cleanup files (whitespace)
find . -type d -name '.git' -prune -o -type f -print0 | xargs -0r sed -i -e 's/[ \t]*$//'
for f in src/**/*.[chS]; do expand -t 8 $f > $f.tmp; mv $f.tmp $f; done
